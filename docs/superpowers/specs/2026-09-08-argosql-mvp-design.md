...
Query Store plan XML carries no XML declaration. Measured: the text begins at `<ShowPlanXML xmlns=...`. Write it out as it came, UTF-8 without BOM, and do not synthesize a declaration. The result is a conformant XML document: the declaration is optional, and with no declaration and no BOM the encoding is UTF-8 by definition, so nothing in the file can contradict its own bytes.

The trap the source warned about is the opposite case and does not arise here. SSMS writes its own `.sqlplan` exports as UTF-16LE with `encoding="utf-16"`; rewriting that content as UTF-8 while keeping the declaration produces a file that lies about itself. That is what the artifact rule below is for, and it applies only where a declaration already exists. Since `asq` never reads a file SSMS wrote, the rule is inert on this path.
...
5. Export a plan larger than 80 KiB and a module body larger than the preview cell limit, and compare the artifact byte for byte against the server value. Test large XML, malformed XML, artifact collisions, disk failure, and the collection cap. Parse every exported plan back as XML and assert it is well formed, no BOM, no declaration.
